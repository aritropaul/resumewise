// classic — sans, hairline rule headings. Honors every theme knob; signature
// font (Inter) is a default suggestion only.
...
export const CLASSIC_BASELINE: NodeStyleMap = {
  name: { fontWeight: "semibold", textCase: "uppercase", letterSpacing: 1.4, color: "ink" },
  label: { color: "muted" },
  contact: { color: "muted" },
...
function buildStyles(theme: ResumeTheme) {
  ...
  const nameN = resolveNodeStyle(theme, "name", CLASSIC_BASELINE);
  const labelN = resolveNodeStyle(theme, "label", CLASSIC_BASELINE);
  const contactN = resolveNodeStyle(theme, "contact", CLASSIC_BASELINE);
  const sectionN = resolveNodeStyle(theme, "section", CLASSIC_BASELINE);
  const roleN = resolveNodeStyle(theme, "role", CLASSIC_BASELINE);
  const datesN = resolveNodeStyle(theme, "dates", CLASSIC_BASELINE);
  const locationN = resolveNodeStyle(theme, "location", CLASSIC_BASELINE);
  const p = resolveNodeStyle(theme, "paragraph", CLASSIC_BASELINE);
  const bullet = resolveNodeStyle(theme, "bullet", CLASSIC_BASELINE);
  const linkNode = resolveNodeStyle(theme, "link", CLASSIC_BASELINE);
  ...
  return {
    ...
    styles: StyleSheet.create({
      ...
      bulletDot: { width: 8, color: bullet.color },
      bulletText: {
        flex: 1,
        ...nodeStyleObj(bullet),
      },
...
function Section({ section, ctx }: { section: ResumeAstSection; ctx: Ctx }) {
  ...
  return (
    <View>
      ...
      {section.bullets?.length ? (
        <Bullets items={section.bullets} ctx={ctx} />
      ) : null}
...
export function ClassicTemplate({ ast, theme }: { ast: ResumeAst; theme: ResumeTheme }) {
