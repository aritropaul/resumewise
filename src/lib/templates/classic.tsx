// classic — sans, hairline rule section headings. Walks the block tree in
// source order. No layout synthesis; heading level drives the styling, author
  ...
  if (layout.mode === "split") {
    return (
      <SplitRow
        ...
        right={layout.right}
        style={style}
        rightStyle={layout.tag === "dates" ? styles.dates : undefined}
        tkns={tkns}
        linkBase={link}
      />
    );
  ...
  if (layout.mode === "whole") {
    return (
      <InlineText
        key={keyIndex}
        text={layout.text}
        style={withAlign(style, layout.align)}
        tkns={tkns}
        linkBase={link}
      />
    );
...
export function ClassicTemplate({ ast, theme }: { ast: ResumeDoc; theme: ResumeTheme }) {
