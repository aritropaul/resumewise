// editorial — italic display headings on a hairline rule, single-column.
// Baseline-driven text styles; template-identity decoration (header underline,
// accent section heading) stays in the render layer.
...
export const EDITORIAL_BASELINE: NodeStyleMap = {
  name: { fontWeight: "semibold", fontStyle: "italic", letterSpacing: 0.4, color: "ink" },
  label: { fontStyle: "italic", color: "muted" },
  contact: { color: "muted" },
  section: { fontWeight: "medium", fontStyle: "italic", color: "accent" },
...
function buildStyles(theme: ResumeTheme) {
  ...
  const nameN = resolveNodeStyle(theme, "name", EDITORIAL_BASELINE);
  const labelN = resolveNodeStyle(theme, "label", EDITORIAL_BASELINE);
  const contactN = resolveNodeStyle(theme, "contact", EDITORIAL_BASELINE);
  const sectionN = resolveNodeStyle(theme, "section", EDITORIAL_BASELINE);
  const roleN = resolveNodeStyle(theme, "role", EDITORIAL_BASELINE);
  const datesN = resolveNodeStyle(theme, "dates", EDITORIAL_BASELINE);
  const locationN = resolveNodeStyle(theme, "location", EDITORIAL_BASELINE);
  const p = resolveNodeStyle(theme, "paragraph", EDITORIAL_BASELINE);
  const bullet = resolveNodeStyle(theme, "bullet", EDITORIAL_BASELINE);
  const linkNode = resolveNodeStyle(theme, "link", EDITORIAL_BASELINE);
  ...
  return {
    ...
    styles: StyleSheet.create({
      ...
      contactItem: { marginRight: 10, ...nodeStyleObj(contactN) },
      link: { ...link },
      sectionHeading: {
      ...
      itemSub: { ...nodeStyleObj(roleN), fontWeight: 400, color: t.muted },
...
export function EditorialTemplate({ ast, theme }: { ast: ResumeAst; theme: ResumeTheme }) {
